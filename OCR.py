...
pngs = [f for f in os.listdir(tempDir) if os.path.isfile(os.path.join(tempDir, f))]
for pic in pngs:
	if pic.endswith('.png'):
		combined_pic = tempDir + '/' + pic
		print(combined_pic)
		tesseract = 'tesseract "' + combined_pic + '" "' + combined_pic + '-ocr" -l sqi+eng+srp_latn PDF' 
		print(tesseract)
		os.system(tesseract)
		if (txtfileon.get()==1):
			tesseract = 'tesseract "' + combined_pic + '" "' + combined_pic + '-ocr" -l sqi+eng+srp_latn txt' 
			print(tesseract)
			os.system(tesseract)
	
# combine OCR'd PDFs into one ---------------------------------------------------------------------------------------------------
ocr_pdfs = [f for f in os.listdir(tempDir) if os.path.isfile(os.path.join(tempDir, f))]
merger = PdfFileMerger()
for pdf in ocr_pdfs:
	if pdf.endswith('.pdf'):
		merger.append(tempDir + '/' + pdf)
		print(tempDir + '/' + pdf)
